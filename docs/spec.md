...
第一引数に AOH（必須）を受け取りインスタンスを生成する。`_check_cols` を実行して全カラムリストを保持する。元の AOH は `clone` してインスタンス内部に保持するため、元のデータは変更されない。第二引数にオプションのハッシュリファレンスを渡すことができ、現時点では `as` のみをサポートする。`as $tbl` は `{ as => \$tbl }` の syntax sugar。
