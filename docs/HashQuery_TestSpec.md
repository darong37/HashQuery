...
### No.44 asNull undef をデフォルト値に置換する
...
### No.50 先頭行での $start=-1 は境界でクランプされる
...
### No.57 count_by の集計対象は where 後のテーブル
...
### No.58 max_by でグループ最大値が条件を満たす行を残す
...
### No.59 min_by でグループ最小値が条件を満たす行を残す
...
### No.60 max_by グループ内に有効値がない場合 undef を返す
...
### No.61 min_by グループ内に有効値がない場合 undef を返す
...
### No.66 count_by を having 外で呼ぶとdieする
...
### No.67 max_by を having 外で呼ぶとdieする
...
### No.68 min_by を having 外で呼ぶとdieする
...
### No.69 first_by を having 外で呼ぶとdieする
...
### No.70 last_by を having 外で呼ぶとdieする
...
### No.71 as で alias 変数を使って行の値を参照できる
...
### No.73 select を where より前に書いても同じ結果
...
### No.74 having を where より前に書いても同じ結果
...
### No.79 grep_concat + where で条件絞り込み
...
### No.80 where + having（having は where 後のテーブルを集計）
...
### No.82 =~ で直前コンテキストにマッチするERROR行を抽出
...
### No.83 =~ で直後コンテキストにERRORを含むINFO行を抽出
...
### No.87 チームの最高スコアが90以上のチームのメンバーを取得
...
### No.88 スコア75以上かつチームに2人以上いるメンバーを取得
...
### No.89 スコア75以上のメンバーからチームごとの先頭を取得
...
|---|---|---|---|
| `$col` | 文字列 | 不可 | 対象カラム名 |
| `$pattern` | Regexp | 不可 | マッチングパターン（内部で `s` フラグが自動付与される） |
...
- `$pattern` には内部で `(?s)` を付与する。呼び出し側が `s` フラグを指定しなくても `.` は改行を含む
...
### No.90 指定カラムの値のみが結果に含まれる（単一行・2カラムテーブル）
...
### No.92 多カラムテーブルで指定カラム以外の値が混入しない
