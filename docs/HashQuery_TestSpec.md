...
```perl
my $r = query \@log_tbl,
    where { $_->{msg}->grep_concat(qr/ERROR/, 0, 0) ne '' };
```
...
query \@log_tbl,
    as $tg1,
    where {
        my $s = $tg1->{msg}->grep_concat(qr/ERROR/, 0, 0);
        push @out, $s if $s ne '';
        1;
    };
```
...
query \@log_tbl,
    as $tg2,
    where {
        my $s = $tg2->{msg}->grep_concat(qr/ERROR/, -1, 1);
        push @out, $s if $s ne '';
        1;
    };
```
...
query \@edge_log,
    as $tg3,
    where {
        my $s = $tg3->{msg}->grep_concat(qr/ERROR first/, -1, 1);
        push @out, $s if $s ne '';
        1;
    };
```
...
query \@edge_log,
    as $tg4,
    where {
        my $s = $tg4->{msg}->grep_concat(qr/ERROR last/, -1, 1);
        push @out, $s if $s ne '';
        1;
    };
```
...
query \@log_tbl,
    as $tg5,
    where {
        my $s = $tg5->{msg}->grep_concat(qr/ERROR/);
        push @out, $s if $s ne '';
        1;
    };
```
...
```perl
my $r = query \@null_tbl,
    where { $_->{val}->grep_concat(qr/.+/, 0, 0) ne '' };
```
...
query \@log_tbl,
    as $tg6,
    where {
        my $s = $tg6->{msg}->grep_concat(qr/ERROR/, 0, 0);
        push @out, $s if $s ne '';
        1;
    };
```
...
my $r = query \@log_tbl,
    as $tc78,
    select [qw/line msg/],
    where  { $tc78->{msg}->grep_concat(qr/ERROR/, 0, 0) ne '' };
```
...
```perl
my $r = query \@log_tbl,
    where { $_->{msg}->grep_concat(qr/ERROR/, -1, 0) =~ /start/ };
```
...
| line | msg                     | コンテキスト文字列                                        | =~ /start/ |
|------|-------------------------|------------------------------------------------------------|------------|
| 2    | ERROR connection failed | `"1 INFO  start\n2 ERROR connection failed\n"`             | ○          |
| 4    | ERROR timeout           | `"3 INFO  retrying\n4 ERROR timeout\n"`                    | ×          |
...
```perl
my $r = query \@log_tbl,
    where { $_->{msg}->grep_concat(qr/INFO/, 0, 1) =~ /ERROR/ };
```
...
| line | msg            | コンテキスト文字列                                         | =~ /ERROR/ |
|------|----------------|-------------------------------------------------------------|------------|
| 1    | INFO  start    | `"1 INFO  start\n2 ERROR connection failed\n"`              | ○          |
| 3    | INFO  retrying | `"3 INFO  retrying\n4 ERROR timeout\n"`                     | ○          |
| 5    | INFO  done     | `"5 INFO  done\n"`（末尾のため次行なし）                    | ×          |
...
my $r = query \@log_tbl,
    where {
        my $ctx = $_->{msg}->grep_concat(qr/ERROR/, 0, 1);
        $ctx ne '' && $ctx !~ /retrying/
    };
```
...
| line | msg                     | コンテキスト文字列                                    | ne '' かつ !~ /retrying/ |
|------|-------------------------|--------------------------------------------------------|--------------------------|
| 2    | ERROR connection failed | `"2 ERROR connection failed\n3 INFO  retrying\n"`      | × （retrying が含まれる） |
| 4    | ERROR timeout           | `"4 ERROR timeout\n5 INFO  done\n"`                    | ○                         |
...
my $r = query \@edge_log,
    where {
        my $ctx = $_->{msg}->grep_concat(qr/ERROR/, -1, 1);
        $ctx ne '' && $ctx !~ /fourth/
    };
```
...
| line | msg         | コンテキスト文字列                                          | ne '' かつ !~ /fourth/    |
|------|-------------|--------------------------------------------------------------|---------------------------|
| 1    | ERROR first | `"1 ERROR first\n2 INFO  second\n"`（先頭のためクランプ）   | ○                          |
| 5    | ERROR last  | `"4 INFO  fourth\n5 ERROR last\n"`                           | × （fourth が含まれる）    |
