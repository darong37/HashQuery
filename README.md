...
| 関数 | 役割 |
|---|---|
| `grep_concat($col, $pattern, $start, $end)` | `$col` の値が `$pattern` にマッチした行を起点に、前後の行の `$col` 値を改行区切りで連結した文字列を返す |
...
# 第1引数: 対象列名、第2引数: 正規表現、第3引数: 開始オフセット（省略時0）、第4引数: 終了オフセット（省略時=$start）
